...
import { AISettings, ProviderConfig, HybridDetectionSettings, ProviderType } from '../types';
...
const SettingsModal: React.FC<SettingsModalProps> = ({ isOpen, onClose, onSave, initialSettings }) => {
  ...
  useEffect(() => {
    if (isOpen) {
        ...
        // Set initial tab based on recognition provider
        setActiveTab(initialSettings.recognitionProvider || initialSettings.currentProvider || 'gemini');
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div className="bg-white dark:bg-slate-800 rounded-xl shadow-2xl w-full max-w-lg p-0 border border-slate-200 dark:border-slate-700 overflow-hidden flex flex-col max-h-[90vh]">
             ...
             <div className="bg-white dark:bg-slate-700 p-2 rounded-lg border border-slate-200 dark:border-slate-600">
               ...
               <select
                 ...
                 onChange={(e) => setSettings(prev => ({
                   ...
                   currentProvider: e.target.value as ProviderType // Keep legacy field in sync
                 }))}
        ...
        {/* Tabs */}
        <div className="flex border-b border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800" role="tablist">
             <button
                onClick={() => handleTabChange('gemini')}
                role="tab"
                aria-selected={activeTab === 'gemini'}
                aria-label="Gemini provider settings"
             ...
             <button
                ...
                aria-label="OpenAI provider settings"
             ...
             <button
                ...
                aria-label="Anthropic provider settings"
