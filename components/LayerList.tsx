  ...
  return (
    <div className="w-80 bg-white dark:bg-slate-800 border-l border-slate-200 dark:border-slate-700 h-full flex flex-col transition-colors duration-200 shrink-0 z-40 shadow-xl">
      <div className="p-4 border-b border-slate-200 dark:border-slate-700">
        <h2 className="font-semibold text-slate-800 dark:text-slate-100">Layer Management</h2>
        <p className="text-xs text-slate-500 dark:text-slate-400">Deconstructed Elements</p>
      </div>
      ...
      <div className="p-4 space-y-3 border-b border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50">
        ...
        <div className="flex flex-col gap-2">
            <div className="flex items-center justify-between">
              <span className="text-sm font-medium text-slate-700 dark:text-slate-300">Background Layer</span>
              <div className="flex gap-2">
                  ...
                  {onToggleErasureMode && (
                      <button
                        ...
                        title="Partial Background Erase"
                      >
            ...
            {isErasureMode && onConfirmErasure && (
                <div className="bg-red-50 dark:bg-red-900/10 border border-red-100 dark:border-red-900/30 p-2 rounded text-xs space-y-2 animate-in fade-in slide-in-from-top-1">
                    <p className="text-red-800 dark:text-red-300">
                        Select areas on canvas to erase (multi-select supported).
                    </p>
                    <div className="flex gap-2">
                        <button
                        ...
                        >
                            {isProcessing ? 'Processing...' : 'Confirm Erase'}
                        </button>
                        <button
        ...
        <div className="flex items-center justify-between">
          <span className="text-sm font-medium text-slate-700 dark:text-slate-300">Text Layer ({textCount})</span>
          <button 
        ...
        <div className="flex items-center justify-between">
          <span className="text-sm font-medium text-slate-700 dark:text-slate-300">Visual Elements ({visualCount})</span>
          <button 
        ...
        <div className="p-4 bg-blue-50 dark:bg-blue-900/20 border-b border-blue-100 dark:border-blue-900/50 space-y-3">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-blue-800 dark:text-blue-200">
               Selected: {selectedElement.type === 'VISUAL' ? 'Visual Element' : 'Text'}
            </span>
            <button 
          ...
          {selectedElement.type === ElementType.VISUAL && (
              <>
                 <textarea
                    ...
                    placeholder="Enter instruction (e.g., Remove text, Split element, Change to red...)"
                 ...
                 <div className="flex gap-2">
                    ...
                    >
                        {isProcessing ? 'Processing...' : 'Split/Recognize'}
                    </button>
                    <button
                        onClick={() => onModifyElement(selectedElement.id, instruction || "Remove text")}
                        disabled={isProcessing}
                    ...
                    >
                        {isProcessing ? 'Generating...' : 'Modify/Generate'}
                    </button>
                 </div>
                 ...
                 {(selectedElement as SlideVisualElement).history && (selectedElement as SlideVisualElement).history!.length > 0 && (
                     <div className="pt-2 border-t border-blue-100 dark:border-blue-800/50">
                        <span className="text-[10px] font-semibold text-slate-500 dark:text-slate-400 mb-1 block">History</span>
            ...
            <div className="flex-1 min-w-0 cursor-pointer">
              <div className="flex items-center gap-2 mb-0.5">
                ...
                <span className="font-medium uppercase text-[10px] tracking-wider opacity-60 flex-shrink-0">
                   {el.type === 'TEXT' ? 'Text' : 'Visual'}
                </span>
                {el.type === ElementType.VISUAL && (el as any).customImage && (
                    <span className="text-[10px] bg-purple-100 text-purple-600 px-1 rounded flex-shrink-0">Modified</span>
                )}
              </div>
